...
@can('folder_create')
    <div class="block my-4">
        <h1>
        {{ trans('cruds.menu.level') }}
        </h1>
    </div>

    <div class="block my-4" style="float: right;">
        <a class="btn-md btn-green items-end" href="{{ route('admin.levels.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.global.addLevel') }}
        </a>
    </div>
@endcan
<div class="main-card">
                ...
                <tbody>
                    @foreach($levels as $key => $level)
                        <tr data-entry-id="{{ $level->id }}">
                            ...
                            <td>
                                @foreach($level->files as $key => $media)
...
{{--                                 @can('folder_show')
                                ...
                                @can('folder_edit')
                                    <a class="btn-sm btn-blue" href="{{ route('admin.levels.edit', $level->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
