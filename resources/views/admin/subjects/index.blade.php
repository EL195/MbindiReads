...
@can('folder_create')
    <div class="block my-4">
        <h1>
        {{ trans('cruds.menu.subjects') }}
        </h1>
    </div>

    <div class="block my-4" style="float: right;">
        <a class="btn-md btn-green items-end" href="{{ route('admin.subjects.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.global.newSubject') }}
        </a>
    </div>
@endcan
<div class="main-card">
                ...
                <tbody>
                    @foreach($subjects as $key => $subject)
                        <tr data-entry-id="{{ $subject->id }}">
                            ...
                            <td>
                                @foreach($subject->files as $key => $media)
...
{{--                                 @can('folder_show')
                                ...
                                @can('folder_edit')
                                    <a class="btn-sm btn-blue" href="{{ route('admin.subjects.edit', $subject->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
