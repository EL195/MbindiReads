...
@can('folder_create')
    <div class="block my-4">
        <h1>
        {{ trans('cruds.menu.agegroup') }}
        </h1>
    </div>

    <div class="block my-4" style="float: right;">
        <a class="btn-md btn-green items-end" href="{{ route('admin.agegroups.create') }}">
            {{ trans('global.add') }} {{ trans('cruds.global.addAgegroup') }}
        </a>
    </div>
@endcan
<div class="main-card">
            ...
            <table class="stripe hover bordered datatable ">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.global.years_start') }}
                        </th>
                         <th>
                            {{ trans('cruds.global.years_end') }}
                        </th>
{{--                         <th>
...
{{--                                 @can('folder_show')
                                ...
                                @can('folder_edit')
                                    <a class="btn-sm btn-blue" href="{{ route('admin.agegroups.edit', $agegroup->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('folder_delete')
                                    <form action="{{ route('admin.agegroups.destroy', $agegroup->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
